...
class CheckDM8(CheckRDS):
  ...
  def parse_sql_table_options(self, sql: str, table: Table):
    # 达梦数据库表定义：COMMENT 'table comment'
    if not sql:
      return
    remaining_sql = sql
    while remaining_sql != "":
  ...
  def parse_sql_table_struct(self, column_sql: str, table: Table):
    ...
    if first_token == "CLUSTER":
      ...
      # CONSTRAINT constraint_name FOREIGN KEY (column_names) REFERENCES table_name (column_names)
      tokens, remaining_sql = next_tokens(remaining_sql, 3)
      ...
      # column_name column_type column_identity column_null column_default column_comment
  ...
  def parse_sql_create_unique_index(self, sql: str, db: Database):
    # 唯一索引语法: CREATE UNIQUE INDEX IF NOT EXISTS index_name ON table_name (column, ...)
    remaining_sql = sql
  ...
  def parse_sql_create_index(self, sql: str, db: Database):
    # 普通索引语法: CREATE INDEX IF NOT EXISTS index_name ON table_name(column, ...)
    remaining_sql = sql
    tokens, remaining_sql = next_tokens(remaining_sql, 5)
    if (
      len(tokens) != 5
