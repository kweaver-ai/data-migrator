#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DM8 校验实现 - 继承 DM8Dialect，添加 check-specific 校验逻辑"""
...
class CheckDM8(DM8Dialect, CheckRDS):
    ...
    def check_init(self, sql_list: list):
        ...
        for sql in sql_list[1:]:
            ...
            if token == "SET":
                ...
                raise Exception(f"不合法的sql语句, 仅支持 'SET SCHEMA', 'CREATE TABLE', 'INSERT': {sql}")
        ...
        for sql in sql_list[1:]:
            ...
            if token == "SET":
                ...
                if token2 == "TABLE":
                    ...
                    raise Exception(f"不合法的sql语句, 仅支持 'CREATE TABLE': {sql}")
            elif token in ("INSERT", "DELETE", "UPDATE"):
                continue
            elif token == "DROP":
                token2, remaining_sql = next_token(remaining_sql)
        ...
        for idx_name, index in table.Indices.items():
            for col_name in index.Columns:
                ...
                if column.ColumnType == "TEXT":
                    raise Exception(
                        f"表 '{table.TableName}' 索引 '{idx_name}' 中字段 '{col_name}' 是文本类型, 建议使用 VARCHAR"
                    )
