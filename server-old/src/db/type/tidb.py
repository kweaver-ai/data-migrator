...
class TiDBRDS(MysqlRDS):
  ...
  def __init__(self, rds_info: RDSInfo, logger: Log):
    ...
    self.RENAME_COLUMN_SQL = "ALTER TABLE {db_name}.{table_name} CHANGE COLUMN IF EXISTS {column_name} {new_name} {column_property} COMMENT '{column_comment}'"
  ...
  def init_db_config(self):
    ...
    try:
      ...
      with self._root_conn_db.cursor() as cursor:
        ...
        cursor.execute("set global sql_mode='STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';")
